 ...
 */
void mouse_check(SpriteCore *mouse_p, Ps2Core *ps2_p, SpriteCore *ghost_p, FrameCore *frame_p, OsdCore *osd_p, int *timer_value, uint32_t speed) {
   int mx, my , gx, gy;
   char ch;
   unsigned long last;
   int xdis, ydis;
   ...
   /*	if(speed <= 299 && speed > 100	 )
	   ...
	   if (ps2_p->get_kb_ch(&ch)) {
   		...
   		if(xdis <= 10 && ydis<= 10) // if ghost and mouse touch clear screen
   		{
...
int main() {
   ...
   while (1) {
     ...
     // do
      ...
      mouse_check(&mouse,&ps2,&ghost, &frame,&osd,&timer_value,speed);
